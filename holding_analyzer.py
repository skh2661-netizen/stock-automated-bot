...
def evaluate_holdings(holdings: List[Dict], features_map: Dict[str, CandidateFeature]) -> List[Dict[str, Any]]:
    ...
    for h in holdings:
        ...
        # [핵심] 시장 폭락으로 스캐너에서 튕겨나간 보유종목 강제 생존(Fallback) 조회
        if not cf:
            try:
                start_dt = (datetime.datetime.now() - datetime.timedelta(days=10)).strftime("%Y-%m-%d")
                fallback_df = fdr.DataReader(code, start_dt)
                if fallback_df is not None and not fallback_df.empty:
                    current_price = float(fallback_df['Close'].iloc[-1])
                    return_pct = round((current_price / entry_price - 1) * 100, 2)
                    
                    action = "HOLD"
                    reason = "기본 보류 (상세 지표 수집 실패)"
                    if return_pct <= -10.0:
                        action = "STOP_LOSS"
                        reason = "수동 손절선(-10%) 이탈"
                    ...
                    results.append({"code": code, "name": name, "entry_price": entry_price, "current_price": current_price, "return_pct": return_pct, "action": action, "reason": reason})
                    continue
            except Exception:
                pass
            results.append({"code": code, "name": name, "entry_price": entry_price, "current_price": 0, "return_pct": 0.0, "action": "HOLD", "reason": "데이터 수집 완전 실패"})
            continue
