...
async def send_message(text):
    if not TELEGRAM_TOKEN or not TELEGRAM_CHAT_ID: 
        print("⚠️ 텔레그램 토큰 또는 방 번호가 설정되지 않았습니다.")
        return False
...
def format_scan_messages(run_type, result):
    ...
    msg_list = []
    header = f"🎯 <b>V8.8.31 DAILY QUANT REPORT</b>\n\n"
    ...
    # [교정] 최상단 요약에 왜 높은 순위(LEVEL 3)를 받았는지 명확한 근거 노출
    ...
    for idx, c in enumerate(candidates[:10], 1):
        ...
        # [교정] 1회 탐색이지만 평균 순위 상위권(<=3위)일 때의 예외적 초기 강세 해석 반영
        if top10_count >= 5: block += f"강한 유지력 ({top10_count}회 출현)\n"
        elif top10_count >= 2: block += f"관심 유지 ({top10_count}회 출현)\n"
        elif avg_rank <= 3.0 and top10_count == 1: block += f"초기 강세 포착 ({top10_count}회 출현)\n"
        else: block += f"신규 진입 후보 ({top10_count}회 출현)\n"
            
        if avg_rank: block += f"평균 순위: {round(avg_rank, 1)}위\n"
        block += "\n"
        ...
        # [교정] LEVEL 2 이하는 구체적 가격 계획을 숨기고 '관찰 계획' 블록으로 엄격 차단
        if ready.get("level") in ["LEVEL 3", "LEVEL 4"]:
            ...
            block += f"📌 <b>관찰 계획</b>\n"
            block += f"조건 충족 후 매매 결성선 도달 시 가격 계획 개방\n"
            block += f"현재 상태: 상승 힘은 확인되었으나 수급 및 지속성 검증 단계\n\n"
