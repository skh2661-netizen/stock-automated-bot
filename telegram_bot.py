...
def format_scan_messages(result, holdings_data=None, p_state=None):
    ...
    if not alert_candidates and not holdings_data:
        return ["⚠️ 조건 충족 V9.2 종목 없음 (시스템 대기)"]
        
    msg_list = []
    level_icons = {"LEVEL 4": "🟢", "LEVEL 3": "🟡", "LEVEL 2": "🟠", "LEVEL 1": "🔴", "LEVEL 0": "❌"}
    
    msg = f"📊 <b>V9.2 실전 퀀트 운용 보고서</b>\n\n"
    msg += f"<b>[1] 🌐 시장 요약 ({market.get('state', 'NORMAL')})</b>\n"
    msg += f"• KOSPI 1D: <b>{market.get('kospi_1d', 0.0)}%</b>\n"
    
    if breadth.get('trend') == 'Unknown':
        msg += f"• 시장 폭(Breadth): <b>⚠️ 일시 수집 실패 ({breadth.get('error_detail', 'Unavailable')})</b>\n"
    else:
    ...
    if alert_candidates:
        ...
        if len(alert_candidates) > 1:
            for idx, c in enumerate(alert_candidates[1:6], 2):
                cd = c["decision"]
                crf = c["raw_features"]
                c_icon = level_icons.get(cd['level'], "⚪")
                msg += f"{idx}위. {c_icon} <b>{c['name']}</b> | {cd['primary_strategy']}\n"
                msg += f" └ Comp <b>{cd['composite_rank']}</b> | Conf {cd['confidence']} | RS20 {round(crf.mom.rs_20d, 2)}\n"
        else:
            msg += "• 후순위 후보 없음\n"
        msg += "━" * 16 + "\n\n"
    
    msg += f"<b>[4] 💼 포트폴리오 자금 관리</b>\n"
    if p_state:
        msg += f"• PHS 건강도: <b>{p_state.phs_score}점</b> | 태세: <b>{p_state.tier}</b>\n"
        used_slots = len(holdings_data) if holdings_data else 0
        msg += f"• 슬롯 현황: <b>{used_slots} / 5 Slots</b> | 현금비중: <b>{max(100 - (used_slots * 15), 5)}%</b>\n\n"
        
    if holdings_data:
        for h in holdings_data:
            h_icon = "🚨" if "청산" in h['judgment'] else "🟢"
            msg += f"{h_icon} <b>{h['name']}</b> | 수익률: {h['pnl']}% | Conf: {h['conf']}점\n"
            msg += f" └ 판정: {h['judgment']} | 대피 손절선: {h['stop_p']:,}원\n"
    else:
        msg += "• 등록된 보유종목 없음\n"
