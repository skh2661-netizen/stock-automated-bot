...
def evaluate_features(
    ...
    for cf in features_list:
        ...
        if not is_holding_eval and cf.chg >= cf.risk.chg_limit:
            continue
        if not is_holding_eval and cf.struc.dist_ma20 > QuantConfig.MA20_MAX_GAP_PCT:
            continue
        if not is_holding_eval and cf.pat.has_long_upper_shadow:
            continue

        strats, bayesian_win_rate = assign_strategies(cf, sys_state)
        if not is_holding_eval and cf.chg < -8.0 and "과대낙폭반등" not in strats:
            continue
...
def rank_features(
    ...
    qz_norms = 100.0 / (1.0 + np.exp(-df_valid["quality_z"].values / QuantConfig.SIGMOID_TEMP))
    liq_norms = 100.0 / (1.0 + np.exp(-df_valid["liq_z"].values / QuantConfig.SIGMOID_TEMP))
    risk_norms = 100.0 / (1.0 + np.exp(-df_valid["risk_z"].values / QuantConfig.SIGMOID_TEMP))
