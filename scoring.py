...
# [수정] P1: 4중 데이터 공급망 (FDR -> 네이버 API -> 로컬 캐시 DB -> 빈 데이터프레임)
def get_krx_retry():
    # Layer 1: FinanceDataReader 기본 호출
    for i in range(2):
        try:
    ...
    # Layer 2: 네이버 모바일 금융 API 우회 및 HTML 인터셉터 검증
    print("🔄 [폴백 레이어 2] 네이버 모바일 API 우회 전술 가동")
    try:
        ...
        print(f"🚨 [폴백 레이어 2 실패] 네이버 우회 통신 장애: {e}")

    # Layer 3: 최악의 차단 장세 대비 로컬 SQLite 최신 데이터 캐시 복원
    print("🔄 [폴백 레이어 3] 통신 전면 마비 상태 확인. 로컬 캐시 자산 데이터 복원을 시도합니다.")
    if os.path.exists(DB_PATH):
        try:
            conn = sqlite3.connect(DB_PATH)
            # 가장 최근에 기록된 스캔 종목의 마스터 스냅샷 로드
            ...
            if not cache_df.empty:
                print(f"✅ [로컬 복원 성공] 캐시 데이터 {len(cache_df)}건 로드 완료.")
                return cache_df
        except Exception as cache_err:
            print(f"🚨 [폴백 레이어 3 실패] 로컬 캐시 DB 훼손: {cache_err}")

    # Layer 4: 파이프라인 무조건 살려내는 빈 데이터 프레임 계약 이행
    print("❌ [최종 장애] 모든 공급원 차단. 빈 데이터 컨텍스트를 공급합니다.")
