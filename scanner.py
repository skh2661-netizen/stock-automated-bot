...
def calculate_trade_plan(price, buy_p, score, ma_gap):
    if score >= 80 and ma_gap <= 10:
        target1, target2 = price * 1.10, price * 1.18
    elif ma_gap <= 5:
        target1, target2 = price * 1.08, price * 1.15
    elif ma_gap <= 15:
        target1, target2 = price * 1.05, price * 1.10
    else:
        target1, target2 = price * 1.03, price * 1.06
...
async def scan_market(run_type="OPEN_SCAN"):
    try:
        ...
        stats = {"total": len(krx), "pass1": 0, "fail_heat": 0, "fail_score": 0, "final": 0, "data_error": False}
        ...
        for _, row in candidates.iterrows():
            ...
            ma_gap, cp_val = (curr['Close'] - ma20) / ma20 * 100, (curr['Close'] - curr['Low']) / (curr['High'] - curr['Low'] + 0.0001) * 100
            ...
            if run_type == "PRE_OPEN": score = calculate_preopen_score(row['Amount'], vr, changes, shadow_ratio, cp_val, rs_1d, 1)
            elif "BREAKOUT" in run_type: score = calculate_breakout_score(row['Amount'], vr, changes, rs_1d, 1)
            else: score = calculate_close_score(row['Amount'], vr, changes, shadow_ratio, 0, cp_val, rs_1d, 1, ma_gap)
        ...
        for item in results:
            save_candidate(run_type, item['code'], item['name'], item['score'], item['buy_p'], item['target_1'], item['target_2'], item['stop_p'], item['price'], item['chg'], item['ma_gap'], item['prime_score'], item['prime_final'], item['conviction'], item['amount_strength'], item['rs_1d'], item['rs_5d'], item['rs_20d'], 1 if item['is_prime_leader'] else 0, 1)
