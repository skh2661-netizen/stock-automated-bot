...
async def scan_market(run_type="OPEN_SCAN"):
    ...
    for _, row in candidates.iterrows():
        ...
        ma_gap = (curr['Close'] - hist['Close'].rolling(20).mean().iloc[-1]) / hist['Close'].rolling(20).mean().iloc[-1] * 100
        ...
        save_candidate(run_type, code, row['Name'], score, buy_p, t1, t2, stop, int(curr['Close']), round(changes, 2), round(ma_gap, 2), 0,0,0,0,0,0,0,0,0)
    ...
    print(f"[SCAN DEBUG] 최종 후보:{len(results)}")
    return {"market": {"kospi": 0, "kosdaq": 0, "mode": run_type, "risk_pct": 0}, "stats": {"total": len(krx), "pass1": len(candidates), "final": len(results)}, "candidates": results}
