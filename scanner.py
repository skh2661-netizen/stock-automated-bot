...
class ScannerConfig:
    ...
    # 전체 다운로드 전, PriceCache(당일 시세)만으로 거르는 1차 컷.
    # 원래 필터(mixed_tval = 오늘 70% + 20일평균 30%)보다 느슨하게 잡아서
    # 최종 판정은 여전히 build_candidate_feature의 원본 로직이 하도록 둔다.
...
def build_price_cache() -> Dict[str, Dict[str, Any]]:
    ...
    - run_scanner의 1차 필터(다운로드 대상 축소)에 사용
    - main.py에서 holding_analyzer에도 그대로 넘겨서, 보유 종목 현재가 조회 시
      종목별로 추가 FDR 호출을 하지 않도록 한다.
    실패해도 빈 dict를 반환해서 파이프라인이 죽지 않게 한다.
    """
    ...
    cache: Dict[str, Dict[str, Any]] = {}
    for attempt in range(2):  # FDR 일시 장애(403/429 등) 대비 1회 재시도
        try:
            cache = _try_load()
            if cache:
                break
...
def build_candidate_feature(args: Tuple) -> Tuple[str, Optional[CandidateFeature], Dict[str, float]]:
    """기존 TA/패턴/Feature 계산 로직 100% 동일. 유일한 차이는 df를 인자로
    미리 받는다는 것 뿐이다 (기존에는 이 함수 안에서 FDR을 직접 호출했음)."""
...
def run_scanner(market_ctx: Dict, price_cache: Optional[Dict[str, Dict[str, Any]]] = None) -> List[CandidateFeature]:
    _logger.info("Starting target generation (PriceCache pre-filter + ThreadPool I/O + multiprocessing CPU)")

    # price_cache가 없으면(예: 단독 실행/테스트) 이 함수 안에서 즉석으로 만든다.
    # -> 기존처럼 인자 없이 호출해도 그대로 동작(하위 호환).
    ...
    # 1) PriceCache 기반 1차 컷: 원본 필터보다 느슨한 기준으로만 후보를 줄인다.
    #    최종 통과 여부는 여전히 build_candidate_feature의 원본 로직이 결정한다.
    valid_items = []
    for code, meta in price_cache.items():
        if (CONFIG.MIN_PRICE <= meta['price'] <= CONFIG.MAX_PRICE and
                meta['volume'] >= CONFIG.MIN_VOLUME and
                meta['trading_value_100m'] >= CONFIG.PRE_FILTER_MIN_TVAL_100M):
    ...
    # 2) ThreadPoolExecutor로 종목별 일봉 다운로드 병렬화 (네트워크 I/O 병목 해소)
    ...
    # 3) 확보된 데이터로 CPU 연산(Feature 계산)은 기존과 동일하게 multiprocessing.Pool 사용
    reject_counts = {"FETCH_FAIL": 0, "INVALID_DATA": 0, "LOW_PRICE": 0, "LOW_VOL": 0, "LOW_TVAL": 0, "MA60_DOWN": 0, "DATA_LACK": 0, "VOL_DRY": 0, "DATA_CORRUPT": 0, "TA_CALC_FAIL": 0, "INDICATOR_FAIL": 0, "RS_FAIL": 0, "PIVOT_FAIL": 0, "PATTERN_FAIL": 0, "PASS": 0}
