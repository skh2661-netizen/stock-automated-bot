...
def _safe_submit(fn, is_orchestrator=False, *args, **kwargs):
    ...
    for _ in range(2):
        try:
            ...
            last_err = e
            exe._shutdown = True 
    _logger.exception("Failed to submit task: %s", last_err, extra={'ctx': 'EXEC'})
...
def _get_session():
    ...
    if not hasattr(local, "session_data") or (now - local.session_data["created"] > 3600):
        if hasattr(local, "session_data"):
            with _session_lock:
                _old_sessions.append((local.session_data["session"], now))
        with _session_lock:
            active = []
...
# =========================================================
# 8. Data Extraction Functions (API, DOM, FDR)
# =========================================================
...
def _fetch_api_raw(ctx: Dict) -> Dict:
    ...
    if kp_sum >= (CONFIG.QUAL.EXPECTED_KP_TOTAL * ctx["ratio"]):
        return {"success": True, "data": data}
    return {"success": False, "error": f"Ratio Failed (KP:{kp_sum}, KD:{kd_sum})", "data": {}}
...
def _fetch_dom_raw(ctx: Dict) -> Dict:
    ...
    kp_sum = kp_u + kp_d + kp_s
    kd_sum = kd_u + kd_d + kd_s

    if not _validate_data(data): return {"success": False, "error": "Validation Failed", "data": {}}
    if kp_sum >= (CONFIG.QUAL.EXPECTED_KP_TOTAL * ctx["ratio"]): return {"success": True, "data": data}
    return {"success": False, "error": f"Ratio Failed (KP:{kp_sum}, KD:{kd_sum})", "data": {}}
...
def _fetch_fdr_raw(ctx: Dict) -> Dict:
    ...
    with STATE.lock:
        ...
        if STATE.fdr_col_cache is None or STATE.fdr_col_cache not in krx.columns:
            ...
            if not found_col:
                found_col = next((c for c in krx.columns if any(kw in c for kw in ["등락률", "Change", "Chg", "Rate", "Ratio"])), None)
            if not found_col:
                raise RuntimeError(f"Col Search Fail. Available columns: {list(krx.columns)}")
            STATE.fdr_col_cache = found_col
        col_cache = STATE.fdr_col_cache

    krx_renamed = krx.rename(columns={col_cache: "ChangesRatio"})
    kpi_df = krx_renamed[krx_renamed['Market'] == 'KOSPI']
    kdq_df = krx_renamed[krx_renamed['Market'] == 'KOSDAQ']
...
def load_breadth() -> Dict:
    ...
    with STATE.lock:
        if len(STATE.breadth_cache_gen) > 0:
            ...
            if c["data"] is not None and (c["time_context"] == ctx["is_open"]) and int(time.time()-c["timestamp"]) < ctx["ttl"]:
                return {"success": True, **c["data"]}
...
def get_market_context() -> Dict:
    ...
    idx = f_idx.result(timeout=0.1) if f_idx in done and f_idx.done() else {"success": False}
    b = f_brd.result(timeout=0.1) if f_brd in done and f_brd.done() else {"success": False}
    score, state, reason = calculate_quality(idx, b)
    _logger.info("State=%s | Score=%d | Reason=%s", state, score, reason, extra={'ctx': 'FINAL'})
    return {"state": state, "allow_scan": (state in ["NORMAL", "CAUTION"]), "data_quality": score, "reason": reason, "breadth": b, "kospi_1d": idx.get("kp_1d", 0), "kosdaq_1d": idx.get("kd_1d", 0), "source": b.get("source"), "partial": idx.get("partial", False)}
