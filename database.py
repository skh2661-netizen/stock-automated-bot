...
def _get_current_schema_version(conn) -> int:
    if not _table_exists(conn, "schema_meta"): return 0
...
def _validate_v9_schema(conn):
    ...
    for tbl, cols in tables.items():
        if not _table_exists(conn, tbl): raise RuntimeError(f"SCHEMA_VALIDATION_FAILED: Table {tbl} missing")
        info = {r[1]: r for r in conn.execute(f"PRAGMA table_info({tbl})").fetchall()}
        for col_name, c_type, not_null, pk in cols:
            if col_name not in info: raise RuntimeError(f"SCHEMA_VALIDATION_FAILED: {tbl}.{col_name} missing")
            if info[col_name][2] != c_type: raise RuntimeError(f"SCHEMA_VALIDATION_FAILED: {tbl}.{col_name} TYPE {info[col_name][2]} != {c_type}")
            if info[col_name][3] != not_null: raise RuntimeError(f"SCHEMA_VALIDATION_FAILED: {tbl}.{col_name} NOTNULL {info[col_name][3]} != {not_null}")
            if info[col_name][5] != pk: raise RuntimeError(f"SCHEMA_VALIDATION_FAILED: {tbl}.{col_name} PK {info[col_name][5]} != {pk}")
    ...
    for idx in unique_idx:
        ...
        if cols == ["code", "signal_date", "revision"]:
            ...
            break
    if not found_composite: raise RuntimeError("SCHEMA_VALIDATION_FAILED: signal_master UNIQUE(code, signal_date, revision) missing")
...
def _migrate_v0_to_v3(conn):
    ...
    if "feature_snapshot_json" not in cols: c.execute("ALTER TABLE candidate_history ADD COLUMN feature_snapshot_json TEXT")
    ...
    for code in v8_codes:
        ...
        c.execute('''INSERT INTO signal_master (signal_id, code, signal_date, strategies, revision, identity_origin, created_at)
                     VALUES (?, ?, ?, ?, ?, ?, ?)''', (sig_id, code, "2000-01-01", '["V8_MIGRATED"]', 0, 'V8_LEGACY_UNIFIED', get_kst_now()))
    ...
    if _table_exists(conn, "signal_outcome"):
        ...
        for row in old_outcomes:
            ...
            c.execute('''INSERT INTO signal_outcome_new (id, history_id, signal_id, code, name, signal_date, price_at_signal, after_1d_chg, after_3d_chg, after_5d_chg, max_gain, max_drawdown, evaluation_status, market_regime) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''', (
                rd["id"], hid, sig_id, code, rd.get("name"), rd.get("signal_date"), rd.get("price_at_signal"),
                rd.get("after_1d_chg"), rd.get("after_3d_chg"), rd.get("after_5d_chg"), rd.get("max_gain"), rd.get("max_drawdown"),
                rd.get("evaluation_status"), rd.get("market_regime")
            ))
    ...
    if cnt_reg_old != cnt_reg_new: raise RuntimeError("MIGRATION_AUDIT_FAILED: Registry count mismatch")
    if cnt_hist_new < cnt_hist_old: raise RuntimeError("MIGRATION_AUDIT_FAILED: History count dropped") 
    if cnt_out_expected != cnt_out_new: raise RuntimeError("MIGRATION_AUDIT_FAILED: Outcome count mismatch")
...
def bootstrap_db():
    ...
    try:
        ...
        if mode.lower() != "wal": raise RuntimeError("SQLITE_WAL_MODE_NOT_ACTIVE")
        ...
        if current_ver < DB_SCHEMA_VERSION:
            ...
            if conn.execute("PRAGMA foreign_key_check").fetchall(): raise RuntimeError("PRE_COMMIT_FK_VIOLATION")
            if conn.execute("PRAGMA integrity_check").fetchone()[0].lower() != "ok": raise RuntimeError("PRE_COMMIT_INTEGRITY_FAILED")
...
def save_signal_transition(master_data: dict, registry_data: dict, log_data: dict) -> bool:
    if master_data and master_data["code"] != registry_data["code"]: raise ValueError("CROSS_TABLE_IDENTITY_MISMATCH: Master vs Registry code")
    if log_data["code"] != registry_data["code"]: raise ValueError("CROSS_TABLE_IDENTITY_MISMATCH: Log vs Registry code")
    ...
    try:
        ...
        if not master_exists and reg_state: raise RuntimeError(f"DB_CORRUPTION: Registry exists but Master missing for {sig_id}")
            
        new_state = registry_data["signal_state"]
        if not is_valid_transition(reg_state, new_state): raise ValueError(f"STATE_MACHINE_VIOLATION: {reg_state} -> {new_state}")
...
# (V8 함수 유지: save_candidate_history, register_signal_outcome...)
