...
def migrate_db():
    tables = {
        "candidates": {"engine_version": "TEXT DEFAULT 'V8.8.19'"},
        "candidate_history": {"engine_version": "TEXT DEFAULT 'V8.8.19'"},
...
def get_signal_persistence(code):
    ...
    five_days_ago = (kst_now - timedelta(days=5)).strftime("%Y-%m-%d")
    today = kst_now.strftime("%Y-%m-%d")
    
    c.execute("SELECT COUNT(*), COUNT(DISTINCT SUBSTR(scan_datetime, 1, 10)), MIN(rank_position), SUM(is_leader), AVG(prime_final), AVG(rank_position) FROM candidate_history WHERE code = ? AND scan_datetime >= ?", (safe_code, five_days_ago))
    row = c.fetchone()
...
def get_top10_stability(code):
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("""SELECT COUNT(*), COUNT(DISTINCT SUBSTR(scan_datetime,1,10)), AVG(rank_position) FROM top10_tracking WHERE code=?""", (safe_code,))
    row = c.fetchone()
    conn.close()
    return {"top10_count": row[0] if row and row[0] else 0, "days": row[1] if row and row[1] else 0, "avg_rank": round(row[2], 1) if row and row[2] else 0.0}
...
def save_candidate_history(scan_datetime, run_type, code, name, rank_position, price, chg, prime_final, prime_score, conviction, rs_1d, rs_5d, rs_20d, ma_gap, amount, amount_strength, risk_level, is_leader=0):
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('''INSERT INTO candidate_history (scan_datetime, run_type, code, name, rank_position, price, chg, prime_final, prime_score, conviction, rs_1d, rs_5d, rs_20d, ma_gap, amount, amount_strength, risk_level, is_leader, created_at, engine_version) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'V8.8.19')''',
