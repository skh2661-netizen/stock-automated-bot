...
def fetch_latest_candidate_data(keyword):
    ...
    c = conn.cursor()
    try:
        query_str = "code = ?" if keyword.isdigit() and len(keyword) == 6 else "name = ?"
        c.execute(f'''
            SELECT code, name, price, chg, prime_final, prime_score, conviction, 
                   rs_20d, ma_gap, risk_level, scan_datetime, is_leader
...
def fetch_pattern_stats(code):
    ...
    try:
        c.execute("""
            SELECT COUNT(*), AVG(after_5d_chg), AVG(max_gain), AVG(max_drawdown)
            FROM signal_outcome 
        ...
        if row and row[0] > 0:
            total = row[0]
            avg5, avg_gain, avg_dd = row[1], row[2], row[3]
            
            c.execute("SELECT COUNT(*) FROM signal_outcome WHERE code = ? AND evaluation_status = 'COMPLETED' AND after_5d_chg > 0", (code,))
            wins = c.fetchone()[0]
...
async def detail_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await update.message.reply_text("⚠️ 종목명이나 코드를 입력하십시오.")
        return
    ...
    if not data:
        await update.message.reply_text(f"❌ 최근 60일 내 [{keyword}] 스캔 기록이 없습니다.")
        return
        
    (code, name, price, chg, p_final, p_score, conv, rs20, ma_gap, risk, scan_dt, is_leader) = data
    memory = get_signal_persistence(code)
    
    msg = f"👑 <b>PRIME WATCH 상세 분석</b>\n" if is_leader else f"🔎 <b>{name} ({code}) 상세 분석</b>\n"
    ...
    # [수정 5] 폭락장(Risk 3) 셧다운 문구 출력
    if risk == 3 and rs20 > 20:
        msg += f"등급: 🔥 <b>시장 생존 리더</b>\n"
        msg += f"매매: ❌ <b>즉시 진입 금지</b>\n"
        msg += f"조건: 시장 안정 확인 후 분할 접근\n\n"
    ...
    if pattern and pattern[4] > 0:
        ...
        # [수정 4] N값 표본 수에 따른 통계적 신뢰도 등급화 부여
        if matches < 5: reliability = "❌ 통계 표본 부족 (기각)"
        elif matches < 20: reliability = "🟡 참고 수준"
        elif matches < 50: reliability = "🟢 유효 패턴"
        else: reliability = "🔥 강한 신뢰"
        
        msg += f"패턴 신뢰도: {reliability}\n"
        ...
        msg += "⚠️ 본 종목의 완료된 T+5 성적표 표본이 부족합니다.\n"

    await update.message.reply_text(msg, parse_mode='HTML')

if __name__ == '__main__':
    if not TELEGRAM_TOKEN:
        print("⚠️ 환경 변수에 TELEGRAM_TOKEN 설정 누락.")
    else:
