...
def fetch_pattern_stats(code):
    ...
    try:
        c.execute('''
            SELECT win_rate, avg_after_5d, avg_max_gain, avg_mdd, match_count, search_level
            FROM signal_outcome 
...
async def detail_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg += f"🤖 <b>유사 패턴 분석 결과:</b>\n"
    
    if pattern and pattern[4] > 0:
        (win_rate, avg_5d, max_gain, mdd, matches, search_lvl) = pattern
        msg += f"과거 표본: {matches}회 (검색 Level {search_lvl})\n"
        msg += f"승률: {win_rate}%\n"
        msg += f"평균 5일 수익: {'+' if avg_5d>0 else ''}{avg_5d}%\n"
        msg += f"최대 상승폭: {'+' if max_gain>0 else ''}{max_gain}%\n"
        msg += f"최대 낙폭: {mdd}%\n"
    else:
        msg += "⚠️ 과거 완료된 동일 패턴 표본이 부족하여 통계 산출 불가.\n"
